...
<%--<script src="/js/jquery.ui.tabs.js" type="text/javascript" language="javascript"></script>--%>
    ...
    <style type="text/css">
        .orginfoTitle, .acctinfoTitle{
            width: 30%;
        }
        .orginfoValue, .acctinfoValue{
            width: 30%;
        }
        .orginfoValue2{
            width: 65%;
        }
        .orginfoErrorInfo, .acctinfoErrorInfo{
            width: 40%;
        }
		...
		<div class="pagecenter" style="width: 50%; float: left; ">
			<c:choose>
				<c:when test="${!empty loginAccount && loginAccount.isSystemDefaultAccount() && loginAccount.isBizAccount()}">
					...
					<c:if test="${!empty errorMsgs['pageError']}">
						<p style="color: red;">${errorMsgs['pageError']}</p>
					</c:if>
				
				
					<form action="/addOrganization" method="post" name="addOrganization">
						<c:import url="organizationInfoFragment.jsp" />
				...
				<c:otherwise>
					You need to login as a bizislife Admin first to add a new organization.				
				</c:otherwise>
			</c:choose>
