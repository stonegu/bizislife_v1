			...
			<fieldset>
				...
				<table>
					...
					<c:if test="${!empty triggers}">
						<c:forEach items="${triggers}" var="trigger" varStatus="triggerIndex">
							<tr>
								<td>${trigger.group}</td>
								<td>${trigger.name}</td>
								<td>${trigger.jobName}</td>
								<td>${trigger.cronExpression}</td>
								<td>${triggerStatus[trigger.name]}</td>
								<td>
									<form action="/quartzController" name="quartzController" method="post">
									
										<c:if test="${triggerStatus[trigger.name] != 'No status'}">
											<c:choose>
												<c:when test="${triggerStatus[trigger.name] == 'Paused status'}">
													<input type="hidden" name="triggerGroupName" value="${trigger.group}"/>
									...
									<form action="/quartzController" name="quartzController" method="post">
										...
										<input type="hidden" name="status" value="deleteJob"/>
							...
							<tr>
								<td colspan="6">
						<p>Reset Expression</p>
								<form action="/quartzController" name="quartzController" method="post">
									<input type="text" name="cronExp" value="${trigger.cronExpression}" />
									<input type="hidden" name="triggerGroupName" value="${trigger.group}"/>
									<input type="hidden" name="triggerName" value="${trigger.name}"/>
									<input type="hidden" name="status" value="changeExp"/>
									<input type="submit" />
								</form>
								</td>
							</tr>
