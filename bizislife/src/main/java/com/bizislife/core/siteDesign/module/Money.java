...
//	}
	...
	public Money plus(Money aThat) {
		...
		return new Money(amount.add(aThat.amount), currency, rounding);
	}
	...
	public Money minus(Money aThat) {
		checkCurrenciesMatch(aThat);
		return new Money(amount.subtract(aThat.amount), currency, rounding);
	}
	...
	public int hashCode() {
		if (fHashCode == 0) {
			...
			fHashCode = HASH_FACTOR * fHashCode + rounding.hashCode();
		}
		return fHashCode;
	}
...
//	private static RoundingMode DEFAULT_ROUNDING;
	private final static RoundingMode DEFAULT_ROUNDING = RoundingMode.HALF_EVEN;
	...
	public Money clone(){
		Money clonedMoney = new Money(amount, currency, rounding);
		return clonedMoney;
	}

	
	public static List<GeneralSelectionType> getAvailableCurrencies(){
//		Set<Currency> availableCurrencies = new HashSet<Currency>();
		List<GeneralSelectionType> availableCurrencies = new ArrayList<GeneralSelectionType>();
		
		Locale[] availableLocales = Locale.getAvailableLocales();
		if(availableLocales!=null && availableLocales.length>0){
			List<String> countryCodes = new ArrayList<String>();
			for(Locale l : availableLocales){
				if(!countryCodes.contains(l.getCountry())){
					countryCodes.add(l.getCountry());
					try{
//						availableCurrencies.add(Currency.getInstance(l));
//						countryCurrencyMap.put(l.getDisplayCountry()+" ["+l.getCountry()+"]", Currency.getInstance(l));
						Currency aCurrency = Currency.getInstance(l);
						if(aCurrency!=null){
							availableCurrencies.add(new GeneralSelectionType(aCurrency.getCurrencyCode(), l.getDisplayCountry()+" ["+l.getCountry()+"]", false));
						}
